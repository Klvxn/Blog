...
class AuthorModelTest(TestCase):
    def setUp(self):
        ...
        self.author = Author.objects.create(
            user=self.user, firstname="test", lastname="user", email="testuser@blogs.xo"
        )
        return super().setUp()
    ...
    def test_edit_author_profile(self):
        ...
        post_response = self.client.post(
            "/author/test-user/edit-profile/", {"bio": "i love running tests"}
        )
        self.assertRedirects(
            get_response, "/users/login/?next=/author/test-user/edit-profile/"
        )
        self.assertRedirects(
            post_response, "/users/login/?next=/author/test-user/edit-profile/"
        )
...
class BlogPostModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.blog = BlogPost.objects.create(
            title=self.title, text=self.text, slug=self.slug, author=self.author
        )
        return super().setUp()
    ...
    def test_edit_blogpost(self):
        response = self.client.post(
            "/post/test-post/1/edit-post", {"title": "don't stop testing"}
        )
...
class CommentModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.comment = Comment.objects.create(
            post=self.blog, username="testuser2", text="test the comment section"
        )
        return super().setUp()
    ...
    def test_string_representation(self):
        ...
        self.assertEqual(
            f"{self.comment.text} by {self.comment.username}",
            "test the comment section by testuser2",
        )
